        ...
        <div className=" min-h-screen mt-5" >
            <div className="grid grid-cols-12">
                        ...
                        <table className=" divide-y divide-gray-200 w-3/4">
                            <thead className="bg-gray-50 p-5">
                                <tr>
                                    ...
                                    <th>{t('seatvip')}</th>
                                    <th>{t('seatbooked')}</th>
                                    <th>{t('seatiordered')}</th>
                                    <th>{t('seatsomeoneisbooking')}</th>
                                </tr>
                            </thead>
                ...
                <div className="col-span-3">
                    ...
                    <p>{t('showdate')}: {thongTinPhim.ngayChieu} - {thongTinPhim.gioChieu}</p>
                    <hr />
                    <div className="flex flex-row my-5">
                        <div className="w-4/5">
                            <span className="text-red-400 text-lg">{t('seat')}</span>
...
export default function CheckoutTab(props) {
    ...
    const operations = <Fragment>
        {!_.isEmpty(userLogin) ? <Fragment> <button className='font-mono text-lg bg-yellow-500 p-2 rounded-md hover:bg-yellow-300 hover:text-white mr-5' onClick={()=>{
            history.push('/profile')
        }}> {userLogin.taiKhoan}</button> <button onClick={()=>{
            localStorage.removeItem(USER_LOGIN);
            localStorage.removeItem(TOKEN);
            history.push('/home');
            window.location.reload();
        }} className="text-blue-800 font-mono font-bold  text-lg hover:text-yellow-500">{t('logout')}</button> </Fragment>: ''} 
    ...
    return <div className="p-2">
        <Tabs tabBarExtraContent={operations} defaultActiveKey="1" activeKey={tabActive} onChange={(key)=>{
           ...
           dispatch({
            ...
            <TabPane tab={t('chooseseat')} key="1" >
                <Checkout {...props} />
            </TabPane>
            <TabPane tab={t('resultbook')} key="2">
                <KetQuaDatVe {...props} />
            </TabPane>
            <TabPane tab={<div className="text-center" style={{display:'flex', justifyContent:'center',alignItems:'center'}}><NavLink to="/"><HomeOutlined style={{marginLeft:5,fontSize:20}} /></NavLink></div>} key="3">
    ...
    const renderTicketItem = function () {
        ...
        return thongTinNguoiDung.thongTinDatVe?.map((ticket, index) => {
            ...
            return <div className="p-2 lg:w-1/2 md:w-1/2 w-full " key={index}>
                <div className="h-full flex items-center font-mono border-gray-200 border p-4 rounded-lg bg-yellow-200">
        ...
        <section className="text-gray-600 body-font">
            <div className="container  py-24 mx-auto">
                <div className="flex flex-col text-center w-full mb-20">
                    <h1 className="sm:text-3xl text-2xl font-mono font-bold  font-medium title-font mb-4  text-yellow-500 ">{t('bookthistory')}</h1>
